        ...
        <form v-if="!waitingOnVerification" action="#" @submit.prevent="handleLogin">
            <div class="overflow-hidden shadow sm:rounded-md max-w-sm mx-auto text-left">
                <div class="bg-white px-4 py-5 sm:p-6">
                    <div>
                        <input type="text" v-maska data-maska="+60 ##-### ####" name="phone" id="phone"
                               v-model="credentials.phone"
                               placeholder="+60 12-345 5678"
                ...
                <div class="bg-white px-4 py-5 sm:p-6">
                    <div>
                        <input type="text" v-maska data-maska="######" v-model="credentials.login_code"
                               name="login_code" id="login_code" placeholder="123456"
                               class="mt-1 block w-full px-3 py-2 rounded-md border border-gray-300 shadow-sm focus:border-black focus:outline-none">
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 text-right sm:px-6">
...
const getFormattedCredentials = () => {
    return {
        phone: credentials.phone.replaceAll('+', '').replaceAll(' ', '').replaceAll('-', ''),
        login_code: credentials.login_code
    }
}
const handleLogin = () => {
    axios.post('http://ridesharing.test/api/login', getFormattedCredentials())
        .then((response) => {
...
const handleVerification = () => {
    axios.post('http://ridesharing.test/api/login/verify', getFormattedCredentials())
        .then((response) => {
